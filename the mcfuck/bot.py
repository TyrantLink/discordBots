...
def logMessages(ctx,type,ctx2='',ext=''):
    ...
    match type:
        case 's':
            ...
            if msgToConsole and not ctx.author.bot and ctx.content != '': print(f'{ctx.author} sent "{modColors}{ctx.content}{colorReset}" in {ctx.channel}{colorReset}')
            sentLog.warning(log)
        case 'd':
            log=f'"{ctx.content}" by {ctx.author} was deleted in {"" if ctx.guild == None else f"{ctx.guild} - "} - {ctx.channel}{ext}'
            if msgToConsole and ctx.content != '': print(f'"{modColors}{ctx.content}{colorReset}" by {ctx.author} was deleted in {ctx.channel}{colorReset}')
        ...
        case 'e':
            ...
            if msgToConsole and ctx.content != '': print(f'{ctx.author} edited "{modColors}{ctx.content}{colorReset}" into "{modColors}{ctx2.content}{colorReset}" in {ctx.channel}{colorReset}')
